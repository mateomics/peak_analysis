...
def write_fasta(peak_sequences, results_dir):
        """
        ...
        """
        for tf_name, sequences in peak_sequences.items():
            # Archivos FASTAs individuales
            with open(os.path.join(results_dir, f'{tf_name}.fa'), 'w') as output_file: # Un archivo por TF
                for index, sequence in enumerate(sequences):  # Enumerar para cada pico
        ...
        print(f'Se han escrito {len(peak_sequences)} archivos FASTA en el directorio de salida.')